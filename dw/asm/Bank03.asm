...
InnCostTbl         = $998C
ShopItemsTbl       = $9991
WeaponsBonusTbl    = $99CF
ArmorBonusTbl      = $99D7
ShieldBonusTbl     = $99DF
BlackPalPtr        = $9A18
OverworldPalPtr    = $9A1A
TownPalPtr         = $9A1C
RedFlashPalPtr     = $9A22
RegSPPalPtr        = $9A24
SplFlshBGPalPtr    = $9A26
BadEndBGPalPtr     = $9A28
EnSPPalsPtr        = $9A2A
CombatBckgndGFX    = $9C8F
SpellCostTbl       = $9D53
ClearWinBufRAM2    = $A788
RemoveWindow       = $A7A2
GetBlockID         = $AC17
ModMapBlock        = $AD66
MapChngFadeNoSound = $B08D
MapChngNoSound     = $B091
MapChngWithSound   = $B097
ResumeMusicTbl     = $B1AE
ChkSpecialLoc      = $B219
CheckMapExit       = $B228
DoJoyRight         = $B252
...
LC0A5:  CMP NPCYCheck           ;
LC0A7:  BNE LC0B0                   ;If not, branch to move the the next NPC sprite data.
...
LC177:  BPL LC172                   ;If not, branch to wait another frame.
LC179:  RTS                     ;
...
LC1D9:  BCC LC1E8                   ;16-bit numbers stored in $3C,$3D
...
LC21B:  DEX                     ;
LC21C:  BNE LC218                   ;Has 4 frames elapsed? If not, branch to wait another frame.
...
LC22B:  BEQ LC238                   ;If not, branch to skip.
...
TransitionMat_0_23: ; [RETRO AI] just the first 24 rows (24*27=648 bytes)
    ...
    LC508: .byte $A9, $00, $85, $08, $20, $06, $C0, $20
...
LC533:  BNE LC52F                   ;If not, branch to wait another frame.
...
LC542:  BEQ LC54F                   ;If not, branch to skip.
...
LC5FB:  AND #$20                ;
LC5FD:  BNE LC603                   ;If not, branch to use address for nametable 1.
...
LC6B4:  INC PPUAddrLB           ;
LC6B6:  BNE LC6BA                   ;Increment PPU target address.
LC6B8:  INC PPUAddrUB           ;
LC6BA: RTS                     ;
...
LC795:  BCS LC79B                   ;Is this a sky tile in the battle scene? If not, branch.
...
TransitionMat_24_26: ; [RETRO AI] the remaining 3 rows (3*27=81 bytes)
    ...
    ; unused byte trail from the original code
    LC83C: .byte $43, $60, $A9, $5F, $85, $08, $20, $F5, $C4, $4C, $90, $C6, $A9, $56, $85
...
LCB6D:  AND #IS_CURSED          ;
LCB6F:  BEQ LCB78                   ;If not, branch to move on.
...
LCC04:  BNE LCC16                   ;If not, branch to check other things.

LCC06:  LDA CharYPos            ;Is player in the right Y coordinate to trigger the end game?
LCC08:  CMP #$08                ;
LCC0A:  BNE LCC16                   ;If not, branch to check other things.
...
LCC53:  BCC LCC57                   ;If so, time to increment her X position.
...
LCCC2:  BNE LCCBE                   ;If not, branch to wait another frame.
...
LCCE4:  BNE LCCE0                   ;If not, branch to wait another frame.
...
LCEB1:  BCS LCEBB                  ;byte? If so, branch.
...
LD069:  BEQ LD072                   ;Branch if player agrees to take Gwaelin along.
...
LD398:  BNE LD394                   ;If not, branch to do another frame.
...
LD4D1:  BNE LD4CD                   ;If not, branch to wait more.
...
LD93A:  BNE LD932                   ;If not, branch to write another byte.
LD93C:  RTS                     ;
...
LD997:  BNE LD98E                   ;If not, branch to shift again.
...
LDA1D:  BNE LDA2A                   ;If so, branch to bring up spell window.
...
LDA53:  BNE LDA5C                   ;If not, branch to move on.
...
LDA7F:  CMP #$07                ;
LDA81:  BNE LDA84                   ;If not, branch to increase the light diameter.
LDA83:  RTS                     ;Else exit.
...
LDB25:  BNE LDB1C                   ;If not, branch to shift again.
...
LDB53:  BNE LDB39                   ;If not, branch to flash again.
LDB55:  RTS                     ;
...
LDBC9:  BCC LDBCD                   ;If not, branch to update HP.
...
LDC05:  BNE LDC09                   ;for the end marker.
...
LDCE5:  BNE LDCE0                   ;If not, loop until something is pressed.
...
LDF80:  BEQ LDF87                   ;If not branch to check for keys.
...
LDF97:  BEQ LDF9F                   ;Branch to upper item if empty.
LDF99:  CLC                     ;
LDF9A:  ADC #$03                ;
...
LDFA2:  AND #$F0                ;
LDFA4:  BEQ LDFAF                   ;Branch to check next byte if empty.
LDFA6:  LSR                     ;
...
LE060:  BNE LE065                   ;If not branch to check next slot.
...
LE06E:  BNE LE073                   ;If not, branch to check if at end of inventory.
...
LE356:  BNE LE34E                   ;If not, branch to get it.
...
LE3D1:  BNE LE3D4                   ;If so, skip drawing the background.
LE3D3:  RTS                     ;
...
LE3E8:  BEQ LE3EE                   ;If so, branch to load outside graphics.
...
LE453:  BNE LE45D                   ;If not branch to load regular enemy graphics.
...
LE4E5:  BNE LE4EB                   ;If not, branch to play enter fight music.
...
LE724:  BNE LE72C                   ;If not, branch to move on.
...
LE72E:  BNE LE736                   ;If not, branch to move on.
...
LE738:  BNE LE751                   ;If not, branch to move on.
...
LE753:  BNE LE76C                   ;If not, branch to move on.
...
LE9A9:  DEX                     ;
LE9AA:  BNE LE9A6                   ;80 frames elapsed? If not, branch to wait more.
...
LE9D3:  BNE LE9CF                   ;If not, branch to delete another tile.
...
LEB0E:  BNE LEB14                   ;No new spell learned. Branch to skip new spell dialog.
...
LEB62:  BNE LEB6B                   ;If so, branch to check which spell.
...
LEB6D:  BNE LEB78                   ;If not, branch to check for heal.
...
LEB7A:  BNE LEB88                  ;If not, branch to check for healmore.
...
LECBA:  CMP EnBaseHP            ;
LECBD:  BCC LECC2                   ;Is new amount higher than max amount? If not, branch.
...
LEE70:  AND #$70                ;
LEE72:  BEQ LEE76                   ;If so, branch. to reset counter.
...
LF002:  BCC LF008                   ;Enemy will do a weak attack if player is much stronger.
...
LF07E:  LDA NMIStatus           ;
LF080:  BNE LF07B                   ;Wait for VBlank to end before continuing.
...
LF102:  BEQ LF10B                   ;If not, branch to exit.
...
RepelTbl:
.incbin "bin/Bank03/RepelTbl.bin"
...
OvrWrldEnGrid:
.incbin "bin/Bank03/OvrWrldEnGrid.bin"
...
CaveEnIndexTbl:
.incbin "bin/Bank03/CaveEnIndexTbl.bin"
...
GFXTilesTbl:
.incbin "bin/Bank03/GFXTilesTbl.bin"
...
LF6D5:  BNE LF6D0                   ;If not, branch to copy another byte.
...
LF6E8:  BNE LF6E1                   ;If not, branch to copy another byte.
...
LF6FF:  BPL LF6F6                   ;If not, branch to verify another character.
...
LF70F:  BPL LF706                   ;If not, branch to check next byte in string.
...
LF724:  BNE LF71A                   ;If not, branch to clear more bytes.
...
LF734:  BCC LF728                   ;If not, branch to clear more bytes.
...
LF75F:  BPL LF755                   ;If not, branch to write the next byte.
...
LFA48:  BCC LFA4C                   ;If not, branch to move on.
...
LFA70:  BPL LFA6A                   ;If so, branch to get next character.
...
LFA7B:  BPL LFA74                   ;If so, branch to get next character.
...
LFABD:  DEY                     ;
LFABE:  BPL LFAB9                   ;Has 32 bytes been written?
...
LFB47:  BPL LFB42                   ;If not, branch to copy next byte.
LFB49:  RTS                     ;
...
LFBBB:  BPL LFBB5                   ;If not, branch to load another character.
...
LFBC6:  BPL LFBBF                   ;If not, branch to load another character.
...
LFBFC:  DEY                     ;
LFBFD:  BPL LFBF5                   ;More bytes to process? If so, branch to continue.
LFBFF:  RTS                     ;
...
LFC17:  BNE LFC13                   ;If not, branch to go to next save game.
...
LFC39:  BCC LFC47                   ;The saved data is run though this function and a 16-bit
...
LFD8C: LDA PPUStatus           ;
LFD8F:  BMI LFD8C                   ;Loop until not in VBlank. Clears the address latch.
LFD91: LDA PPUStatus           ;
LFD94:  BPL LFD91                   ;Wait until VBlank starts.
LFD96: LDA PPUStatus           ;
LFD99:  BMI LFD96                   ;In VBlank.  Bit should be clear on second read.
...
LFE4D:  BNE LFE45                   ;If so, branch to get next byte.
