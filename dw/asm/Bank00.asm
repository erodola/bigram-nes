...
LA794: STA (GenPtr3C),Y        ;Have 256 bytes been cleared?  [RETRO AI] 0x27a4
LA796:  INY                     ;
LA797:  BNE LA794               ;If not, branch to clear another byte.
...
LA903:  BCS LA909                   ;Is this a sky tile in the battle scene? If not, branch.
...
ChkWtrBlkUp:
LAB58: BEQ LAB5B                   ;Is block above target block water? If so, branch.
...
LAB66: BEQ LAB6D                   ;Is block below target block water? If so, branch.
...
LAB9C:  BEQ LABA0                   ;Is the target land? If not, branch.
...
LABAA:  BEQ LABAE                   ;Is the target land? If not, branch.
...
LABCA:  BEQ LABCE                   ;Is the target land? If not, branch.
...
LABD8:  BEQ LABDC                   ;Is the target land? If not, branch.
...
LB168:  DEX                     ;
LB169:  BNE LB164               ;More blank tiles to store? if so, branch to store another one.
...
LB190:  DEX                     ;
LB191:  BNE LB18C               ;More blank tiles to store? if so, branch to store another one.
...
LB236:  BNE LB22C               ;If not, branch to check the next map entry.
...
LB277:  BCC LB27B                   ;Update upper byte of X position, if necessary. 
...
LB285:  BCC LB289                   ;Update upper byte of X position, if necessary. 
...
LB371:  BCS LB375                   ;Update upper byte of X position, if necessary. 
...
LB37F:  BCS LB383                   ;Update upper byte of X position, if necessary. 
...
LB3FD:  BCC LB401                   ;Update upper byte of Y position, if necessary. 
...
LB40B:  BCC LB40F                   ;Update upper byte of Y position, if necessary. 
...
LB529:  BCS LB52D                   ;Update upper byte of Y position, if necessary. 
...
LB537:  BCS LB53B                   ;Update upper byte of Y position, if necessary. 
...
LB547:  BNE LB54D                   ;to the proper maximum value.
LB549:  LDA #$EF                ;
LB54B:  STA ScrollY             ;
...
LB5EA:  BNE LB5F0                   ;If it rolls over, make sure its set to the proper value.
LB5EC:  LDA #$0E                ;
LB5EE:  STA NTBlockY            ;
...
LB6D6:  JMP LB6CE                   ;Has proper direction been found? If not, branch to decrement.
...
LB780:  BNE LB78B                   ;If so, branch to move to next step of calculation.

LB782:  LDA NPCYPos,X           ;Extract the NPC Y location data.
LB784:  AND #$1F                ;Is NPC Y coordinate on screen?
LB786:  BNE LB78B                   ;If so, branch to move to next step of calculation.
...
LB791:  BEQ LB796                   ;If so, branch to keep processing movement for this NPC.
...
LB7BA:  BEQ LB7C2                   ;If not, branch to skip window block check.
...
LB7C9:  BNE LB7D0                   ;If not, branch to check next direction.
...
LB7D2:  BNE LB7D9                   ;If not, branch to check next direction.
...
LB7DB:  BNE LB7E2                   ;If not, branch to move NPC left.
...
LB7E8:  BCS LB7ED                   ;If not, branch to check map width.
...
LB7F8:  BEQ LB800                   ;If not, branch to skip window block check.
...
LB802:  CMP CharXPos            ;
LB804:  BNE LB80C                   ;Is NPC trying to move to player's current position?
...
LB824:  BNE LB832                   ;If not, branch to check the next NPC.

LB826:  LDA _NPCYPos,Y          ;Does this NPC Y position match another NPC Y position?
LB829:  AND #$1F                ;
LB82B:  CMP ThisNPCYPos         ;
LB82D:  BNE LB832                   ;If not, branch to check the next NPC.
...
LB863:  BMI LB868                   ;If so, branch to update NPC movement.
...
LB86A:  BEQ LB86F                   ;If not, branch to keep processing NPC movement.

LB86C:  JMP EndNPCMoveLoop      ;($B8EA)Check for next NPC movement.

LB86F: LDA NPCYPos,X           ;Is NPC facing up?
LB871:  AND #$60                ;
LB873:  BNE LB893                   ;If not, branch to check the next direction.
...
LB895:  BNE LB8B1                   ;If not, branch to check the next direction.
...
LB8B3:  BNE LB8D1                   ;If not, branch to move NPC left.
...
LB944:  BEQ LB94F                   ;If not, branch.

LB946:  LDA WindowBlock         ;Is there a window covering the NPC?
LB948:  CMP #$FF                ;
LB94A:  BEQ LB94F                   ;If not, branch.
...
LB9FD:  AND #$0F                ;
LB9FF:  BEQ LBA02                   ;If so, branch to updater the NPC counter.
LBA01:  RTS                     ;
...
LBA7F:  BMI LBA83                   ;then the NPC may be visible.
...
LBBAA:  DEX                     ;
LBBAB:  BNE LBBA7               ;Have 40 frames passed?
...
LBBB6:  DEY                     ;
LBBB7:  BNE LBBB3               ;Has the whole row been loaded? If not, branch to do another byte.
...
LBD9B:  BNE LBD96               ;Branch if more to repeat.
...
LBDB5:  STA PPUIOReg            ;
LBDB8:  INC PPUAddrLB           ;
LBDBA:  BNE LBDBE                  ;Increment to next address.
LBDBC:  INC PPUAddrUB           ;
LBDBE: RTS                     ;
...
LBDC5:  BNE LBDC9                   ;Increment pointer to next data byte.
