...
TrimZeros_2Digits:              ; last stub falls through
        LDY #4                  ; 2 bytes

;------------ common 18-byte loop -------------------------------
TrimZeros_Loop:
        LDA format_buf-6,Y      ; pick current digit
        CMP #$80                ; is it the ‘0’ tile?
        BNE TrimZeros_RTS       ; first non-zero → done
        LDA #$FF                ; else blank it
        STA format_buf-6,Y
        INY                     ; next more-significant digit
        CPY #5                  ; have we reached -1 yet?
        BNE TrimZeros_Loop
TrimZeros_RTS:
        RTS
...
; Output: ptygen_class + char_index = generated class (1 byte)
; ==================================
GetRandomClass:
  @Retry:
    JSR Rand8             ; get a random number between 0-255
    LDA rand_byte
    AND #$07              ; mask to 0-7 (b111)
    CMP #6
    BCS @Retry            ; if 6 or 7, try again
    LDX char_index
    STA ptygen_class, X   ; assign random class
    RTS
...
PtyGen_DrawOneText:         ; reached from above by fall-through
...
lut_PtyGenBuf:
  .BYTE $00,$00,$00,$00,$00,$00,$07,$0C,$05,$06,$40,$40,$04,$04,$30,$40
  .BYTE $01,$00,$00,$00,$00,$00,$15,$0C,$13,$06,$B0,$40,$12,$04,$A0,$40
  .BYTE $02,$00,$00,$00,$00,$00,$07,$18,$05,$12,$40,$A0,$04,$10,$30,$A0
  .BYTE $03,$00,$00,$00,$00,$00,$15,$18,$13,$12,$B0,$A0,$12,$10,$A0,$A0
...
ShopFrame:                          ; version that shows the cursor
    JSR ClearOAM
    JSR DrawShopPartySprites
    JSR DrawShopCursor
    JMP ShopFrame_Common            ; new jump (3 bytes)

ShopFrameNoCursor:                  ; version without cursor
    JSR ClearOAM
    JSR DrawShopPartySprites
                                    ; fall-through into common code

ShopFrame_Common:                   ; common tail (was duplicated in the old code)
    JSR WaitForVBlank_L
    ...
    LDA #BANK_THIS
    STA cur_bank
    JSR CallMusicPlay               ; keeps music in the right bank

_ShopFrame_CheckBtns:               ; we reach here by fall-through
    LDA joy
    AND #$0F
    STA tmp+7
...
MenuWaitForBtn_SFX:                 ; want the SFX
        JSR     WaitForBtn_Common
        JMP     PlaySFX_MenuSel     ; tail-call, then RTS inside PlaySFX

MenuWaitForBtn:                     ; silent version
        JSR     WaitForBtn_Common
        RTS

WaitForBtn_Common:
WaitLoop:
        JSR     MenuFrame           ; update music, etc.

        LDA     joy_a               ; A|B pressed?
        ORA     joy_b
        BEQ     WaitLoop            ; no - keep waiting

        LDA     #0                  ; yes - clear latches
        STA     joy_a
        STA     joy_b
        RTS
